...
int main(int argc,char* argv[]){
    ...
    builder.make_mq_client(FLAGS_mq_user,FLAGS_mq_pswd,FLAGS_mq_host,   \
        FLAGS_mq_msg_exchange,FLAGS_mq_msg_queue,FLAGS_mq_msg_binding_key);
    builder.makeChannelManager(FLAGS_registry_host,FLAGS_base_service,FLAGS_file_service,FLAGS_user_service);
    builder.makeESUserManager({FLAGS_es_host});
    builder.make_message_service(FLAGS_listen_port,FLAGS_rpc_timeout,FLAGS_rpc_threads);
    builder.make_regClient(FLAGS_registry_host,FLAGS_base_service+FLAGS_instance_name,FLAGS_access_host);

    MessageService::ptr message_service =  builder.build();
