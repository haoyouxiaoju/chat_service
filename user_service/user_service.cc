...
int main(int argc,char* argv[]){
    ...
    builder.makeUserManager(FLAGS_mysql_user, FLAGS_mysql_pswd, FLAGS_mysql_host, 
        FLAGS_mysql_db, FLAGS_mysql_cset, FLAGS_mysql_port, FLAGS_mysql_pool_count);
    builder.makeESUserManager({FLAGS_es_host});
    builder.makeDMSManager(FLAGS_dms_key_id,FLAGS_dms_key_secret);
    builder.makeChannelManager(FLAGS_registry_host,FLAGS_base_service,FLAGS_file_service);
    builder.make_rpc_service(FLAGS_listen_port,FLAGS_rpc_timeout,FLAGS_rpc_threads);
